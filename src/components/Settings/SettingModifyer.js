...
const Option = ({ active, text, callback }) => {
    return (
        <button className={`outline-none flex-1 p-3 px-5 m-auto rounded-lg transition-color duration-300 ${active ? 'shadow-lg bg-sky-400' : ''}`} onClick={callback}>{ text }</button>
    )
}
